...
const Tshirts = ({products}) => {
  return (
    <div>
      <Head>
        <title>Tshirts - Raj's wear</title>
        <meta
          name="viewport"
          content="width=device-width, height=device-height, initial-scale=1.0, maximum-scale=1.0"
        />
      </Head>
      <section className="text-gray-600 body-font justify-center min-h-screen">
        <div className="container  md:px-20 md:py-14 mx-auto justify-center">
          <div className="flex flex-wrap justify-center ">
          {Object.keys(products).length ===0 && <p className="mx-4">Sorry all the Tshirts are currently out of stock. New stock coming soon. Stay Tuned!.</p>}
            {Object.keys(products).map((item)=>{
            
            return <Link passHref={true} key={products[item]._id} href={`/product/${products[item].slug}`}>
              <div className="mx-4 shadow-lg my-2">
                <img
                  alt="ecommerce"
